...
export class AppNotificationComponent implements OnInit, OnDestroy {
  ...
  addReceiveMessageListener() {
    this.hubConnection.on('BroadCastNotificationCount', (connections, sms : number) => {
...
//      //this.hubConnectionBuilder = new HubConnectionBuilder().withUrl('http://192.168.2.87:8021/signalhub/?token='+this._token).configureLogging(LogLevel.Information).build();
...
//      //this.hubConnectionBuilder = new HubConnectionBuilder().withUrl('https://localhost:7145/signalhub/?token='+this._token).configureLogging(LogLevel.Information).build();
     
     
   
   
//     //this.hubConnectionBuilder = new HubConnectionBuilder().withUrl("http://192.168.2.87:8021/signalhub", { accessTokenFactory: () => this._token }).configureLogging(LogLevel.Information).build();
     
//     // this.hubConnectionBuilder = new HubConnectionBuilder().withUrl('https://192.168.2.87:450/signalhub',options).configureLogging(LogLevel.Information).build();
//      this.hubConnectionBuilder.start().then(() => console.log('Connection started.......!')).catch(err => console.log('Error while connect with server'));
//       let connections : string[] = [];
//       //this.hubConnectionBuilder.on('BroadCastNotificationCount', (connections:string[], sms : string) => {
//         this.hubConnectionBuilder.on('BroadCastNotificationCount', (connections, sms : string) => {
//         console.log( " Message : " + sms);
//         //this.user = u;
//           this.message = sms;
//       });
