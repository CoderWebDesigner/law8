...
@Component({
  selector: 'app-security-matter-access-responsable-lawyers',
  templateUrl: './security-matter-access-responsable-lawyers.component.html',
  ...
  implements OnChanges
{
  ...
  source: any[] = [];
  target: any[] = [];
  apiUrls = API_Config.responsibleLawyerSecurity;
  ...
  submit() {
    ...
    let model = {
      userId: this.userId,
      lwersIds: this.target.map((item) => item.id),
    };
    this._apiService
